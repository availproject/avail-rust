use crate::{config::TransactionLocation, error::Error};
...
pub async fn author_rotate_keys(client: &RpcClient) -> Result<SessionKeys, Error> {
	let params = rpc_params![];
...
pub async fn state_get_storage(client: &RpcClient, key: &str, at: Option<H256>) -> Result<Option<Vec<u8>>, Error> {
...
pub async fn chainspec_v1_genesishash(client: &RpcClient) -> Result<H256, Error> {
	let value: String = client.request("chainSpec_v1_genesisHash", rpc_params![]).await?;
	Ok(H256::from_str(&value).map_err(|e| e.to_string())?)
}

pub async fn state_get_metadata(client: &RpcClient, at: Option<H256>) -> Result<Vec<u8>, Error> {
