...
pub async fn get_block_number(client: &RpcClient, at: H256) -> Result<Option<u32>, subxt_rpcs::Error> {
	let params = rpc_params![at];
	let value = client.request("system_getBlockNumber", params).await?;
	Ok(value)
}

pub async fn latest_block_info(client: &RpcClient, use_best_block: bool) -> Result<BlockRef, subxt_rpcs::Error> {
