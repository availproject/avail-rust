...
async fn task(client: Client, account: Keypair, _d: bool) -> ReturnResult {
	let c1 = client
		...
		.balances()
		.transfer_keep_alive(
			avail_rust::constants::dev_accounts::bob().public_key().to_account_id(),
			100,
		)
		.call;
	...
	let c2 = client
		...
		.balances()
		.transfer_keep_alive(
			avail_rust::constants::dev_accounts::dave().public_key().to_account_id(),
			100,
		)
		.call;
	...
	let tx = client.tx().utility().batch(vec![c1, c2]);
