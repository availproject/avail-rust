...
use subxt_rpcs::{self, RpcClientT, UserError};
...
impl CustomRpcClient {
	pub fn new(endpoint: &str) -> Self {
		...
		_ = tokio::spawn(async move { Self::task(client, endpoint, rx).await });
...
impl RpcClientT for CustomRpcClient {
	fn request_raw<'a>(
		&'a self,
		method: &'a str,
...
async fn main() -> Result<(), ClientError> {
	...
	let rpc_client = CustomRpcClient::new(LOCAL_ENDPOINT);
	...
	println!("Block Hash: {:?}, Tx hash: {:?}", receipt.block_ref.hash, receipt.tx_ref.hash);
