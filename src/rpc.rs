...
pub mod system {
	...
	pub async fn chain_type(client: &Client) -> Result<String, ClientError> {
		...
		let value = client
			.rpc_client
			.request("system_chainType", params)
			.await?;
		Ok(value)
	}
	...
	pub async fn node_roles(client: &Client) -> Result<Vec<NodeRole>, ClientError> {
		...
		let value = client
			.rpc_client
			.request("system_nodeRoles", params)
			.await?;
		Ok(value)
	}
	...
	pub async fn properties(client: &Client) -> Result<SystemProperties, ClientError> {
		...
		let value = client
			.rpc_client
			.request("system_properties", params)
			.await?;
		Ok(value)
	}
	...
	pub async fn sync_state(client: &Client) -> Result<SyncState, ClientError> {
		...
		let value = client
			.rpc_client
			.request("system_syncState", params)
			.await?;
		Ok(value)
	}
...
pub mod author {
	...
	pub async fn rotate_keys(client: &Client) -> Result<Vec<u8>, ClientError> {
		...
		let value: Bytes = client
			.rpc_client
			.request("author_rotateKeys", params)
			.await?;
		Ok(value.0)
	}
