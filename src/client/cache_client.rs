...
impl CacheClient {
	...
	pub fn find_signed_block(&self, block_hash: H256) -> Option<Arc<SignedBlock>> {
		let lock = self.inner.read().expect("Should not be poisoned");
		lock.signed_blocks.find(block_hash)
	}

	pub fn push_signed_block(&self, value: (H256, Arc<SignedBlock>)) {
...
struct CachedSingedBlocks {
	blocks: Vec<Option<(H256, Arc<SignedBlock>)>>,
	ptr: usize,
	enabled: bool,
}
