...
pub async fn event_test() -> Result<(), Error> {
	...
	// Bond
	{
		...
		let events = block::Events::new(client.clone(), 2280015).extrinsic(1).await.unwrap();
	...
	// Unbond
	{
		let events = block::Events::new(client.clone(), 1835193).extrinsic(1).await.unwrap();
	...
	// ValidatorPrefsSet
	{
		let events = block::Events::new(client.clone(), 1814105).extrinsic(1).await.unwrap();
	...
	// Chilled
	{
		let events = block::Events::new(client.clone(), 1811904).extrinsic(1).await.unwrap();
	...
	// Rewarded
	{
		let events = block::Events::new(client.clone(), 1861532).extrinsic(1).await.unwrap();
	...
	// Withdrawn
	{
		let events = block::Events::new(client.clone(), 1861093).extrinsic(1).await.unwrap();
